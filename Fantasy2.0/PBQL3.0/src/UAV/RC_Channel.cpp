...
/*-------获得对象属性的方法----------------*/
int16_t RC_Channel::get_the_pwm(){
	return radio_in;
}
int16_t RC_Channel::get_the_control_in(){
	return control_in;
}

int8_t RC_Channel::get_switch_position(){
	if (radio_in < 1231) return 0;
	else if (radio_in < 1361) return 1;
	else if (radio_in < 1491) return 2;
	else if (radio_in < 1621) return 3;
	else if (radio_in < 1750) return 4;
	else return 5;
}

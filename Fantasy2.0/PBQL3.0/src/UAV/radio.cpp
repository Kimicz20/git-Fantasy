...
void Copter::read_radio()
{
    ...
    if (has_new_input==true) {
        ...
        /* 参数重定向 */
        roll_pwm = this->supt->getParamValueWithNameAndKey("set_pwm","roll_pwm");
        pitch_pwm = this->supt->getParamValueWithNameAndKey("set_pwm","pitch_pwm");
        throttle_pwm = this->supt->getParamValueWithNameAndKey("set_pwm","throttle_pwm");
        yaw_pwm = this->supt->getParamValueWithNameAndKey("set_pwm","yaw_pwm");
        mode_pwm = this->supt->getParamValueWithNameAndKey("set_pwm","mode_pwm");

        /* 起始插桩路径记录 */
        start = clock();
        this->supt->setCurProcessResult("set_pwm",start,1);
        ...
        this->supt->setCurProcessResult("set_pwm",end,2);
        this->supt->setCurProcessResult("set_pwm",(end-start),3);

        /* 起始插桩路径记录 */
        start = clock();
        this->supt->setCurProcessResult("set_throttle_and_failsafe",start,1);

        /* 参数重定向 */
        channel_throttle->radio_in = this->supt->getParamValueWithNameAndKey("set_throttle_and_failsafe","radio_in");
        set_throttle_and_failsafe(channel_throttle->radio_in);

        /* 结束插桩路径记录 */
        end = clock();
        this->supt->setCurProcessResult("set_throttle_and_failsafe",end,2);
        this->supt->setCurProcessResult("set_throttle_and_failsafe",(end-start),3);

        /* 起始插桩路径记录 */
        start = clock();
        this->supt->setCurProcessResult("set_throttle_zero_flag",start,1);
        ...
        this->supt->setCurProcessResult("set_throttle_zero_flag",end,2);
        this->supt->setCurProcessResult("set_throttle_zero_flag",(end-start),3);
        ...
        // cout <<"elapsed: "<<elapsed <<"FS_RADIO_TIMEOUT_MS:"<< FS_RADIO_TIMEOUT_MS <<"failsafe_throttle:"<< g.failsafe_throttle <<"radio:"<< failsafe.radio <<endl;

        /* 参数重定向 */
        g.failsafe_throttle=this->supt->getParamValueWithNameAndKey("set_failsafe_radio","g.failsafe_throttle");
        failsafe.radio=this->supt->getParamValueWithNameAndKey("set_failsafe_radio","failsafe.radio");
        int flag = this->supt->getParamValueWithNameAndKey("set_failsafe_radio","flag");

        //模拟 错误 1
        while(flag){

        }
        if (elapsed >= FS_RADIO_TIMEOUT_MS && g.failsafe_throttle && failsafe.radio==0) {
            //Log_Write_Error(ERROR_SUBSYSTEM_RADIO, ERROR_CODE_RADIO_LATE_FRAME);

            /* 起始插桩路径记录 */
            start = clock();
            this->supt->setCurProcessResult("set_failsafe_radio",start,1);
            ...
            this->supt->setCurProcessResult("set_failsafe_radio",end,2);
            this->supt->setCurProcessResult("set_failsafe_radio",(end-start),3);
        }
    }
}

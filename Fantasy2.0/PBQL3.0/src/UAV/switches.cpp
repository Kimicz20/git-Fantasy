...
void Copter::read_control_switch(uint32_t t)
{
	control_switch_state.now_time = t;
    // calculate position of flight mode switch
    /*int8_t switch_position;
    if      ((*channel_mode).radio_in < 1231) switch_position = 0;
	else if ((*channel_mode).radio_in < 1361) switch_position = 1;
	else if ((*channel_mode).radio_in < 1491) switch_position = 2;
	else if ((*channel_mode).radio_in < 1621) switch_position = 3;
	else if ((*channel_mode).radio_in < 1750) switch_position = 4;
    else switch_position = 5;*/
	control_switch_state.switch_position = channel_mode->get_switch_position();

    // store time that switch last moved
	if (control_switch_state.last_switch_position != control_switch_state.switch_position) {
        //control_switch_state.last_edge_time_ms = tnow_ms;
		set_control_switch_last_edge_time();
    }

    // debounce switch
	/*bool control_switch_changed = control_switch_state.debounced_switch_position != control_switch_state.switch_position;
	...
	if ((control_switch_state.debounced_switch_position != control_switch_state.switch_position) && (control_switch_state.now_time - control_switch_state.last_edge_time_ms > CONTROL_SWITCH_DEBOUNCE_TIME_MS) && failsafe.radio == 0 && failsafe.radio_counter == 0) {
        // set flight mode and simple mode setting
		desired_mode = get_desired_mode();
		...
		control_switch_state.debounced_switch_position = control_switch_state.switch_position;
    }

	control_switch_state.last_switch_position = control_switch_state.switch_position;
}
...
{
    control_switch_state.last_switch_position = control_switch_state.debounced_switch_position = 0;
    read_control_switch(clock());
}

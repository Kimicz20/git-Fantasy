...
{
    uint32_t run_started_usec = clock()*1000;
        ...
        if (dt >= interval_ticks) {
            ...
            if (dt >= interval_ticks*2) {
                ...
                if (_debug > 1) {
					...
					std::cout << "Scheduler slip task " << _tasks[i].name << std::endl;
                }
            }
                ...
                if (time_taken > _task_time_allowed) {
                    ...
                    if (_debug > 2) {
                        /*hal.console->printf_P(PSTR("Scheduler overrun task[%u-%s] (%u/%u)\n"),
                                              ...
                                              (unsigned)time_taken,
                                              (unsigned)_task_time_allowed);*/
						std::cout << "Scheduler overrun task " << _tasks[i].name << std::endl;
                    }
                }
...
{
    uint32_t dt = clock()*1000 - _task_time_started;

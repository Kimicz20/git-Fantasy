    ...
    _num_instances(0)
{
	...
	//可设定参数值部分的初始化
	_monitoring[0] = BattMonitor_TYPE_NONE;
	_volt_pin[0] = 2;
	_curr_pin[0] = 3;
	_volt_multiplier[0] = 10.1f;
	_curr_amp_per_volt[0] = 17.0f;
	_curr_amp_offset[0] = 0;
	_pack_capacity[0] = 3300;
}
...
void AP_BattMonitor::init()
{
	...
	/*-----------GC------------------*/
	int monitoring_type_gc;
	std::cout << "sets the monitor type 0=disabled, 3=voltage only, 4=voltage and current" << std::endl;
	std::cin >> monitoring_type_gc;
	set_monitoring(0, monitoring_type_gc);
	/*-----------GC----------------*/
	...
	std::cout << "Batt: Init Done" << std::endl;
}

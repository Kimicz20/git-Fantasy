...
void AP_AHRS_NavEKF::update_EKF1(void)
{
    ...
    //        start_time_ms = clock();
    //    }
    //    if (clock() - start_time_ms > startup_delay_ms) {
...
AP_AHRS_NavEKF::EKF_TYPE AP_AHRS_NavEKF::active_EKF_type(void) const
{
    ...
    //    }
    //    if ( !filt_state.flags.using_gps && _gps.status() >= AP_GPS::GPS_OK_FIX_3D) {
...
bool AP_AHRS_NavEKF::initialised(void) const
{
    ...
    default:
        ...
        return (ekf1_started && (clock() - start_time_ms > AP_AHRS_NAVEKF_SETTLE_TIME_MS));

    case 2:
        // initialisation complete 10sec after ekf has started
        return (ekf2_started && (clock() - start_time_ms > AP_AHRS_NAVEKF_SETTLE_TIME_MS));
    }
};

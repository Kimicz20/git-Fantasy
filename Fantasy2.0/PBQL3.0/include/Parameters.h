...
public:
    ...
    Parameters() :
        ...
        pi_vel_xy               (VEL_XY_P,        VEL_XY_I,                         VEL_XY_IMAX,        VEL_XY_FILT_HZ,     0.020f),
        ...
        p_alt_hold              (ALT_HOLD_P)
    {
		...
		throttle_behavior = 0;
		//serial_manager = AP_SerialManager;
		telem_delay = 0;
		gcs_pid_mask = 0;
		rtl_altitude = RTL_ALT;
		sonar_gain = SONAR_GAIN_DEFAULT;
		failsafe_battery_enabled = FS_BATT_DISABLED;
		fs_batt_voltage = FS_BATT_VOLTAGE_DEFAULT;
		fs_batt_mah = FS_BATT_MAH_DEFAULT;
		failsafe_gcs = FS_GCS_ENABLED_ALWAYS_RTL;
		gps_hdop_good = GPS_HDOP_GOOD_DEFAULT;
		...
		frame_orientation = AP_MOTORS_X_FRAME;
		...
		arming_check = ARMING_CHECK_ALL;
		disarm_delay = AUTO_DISARMING_DELAY;
		//angle_max = DEFAULT_ANGLE_MAX;
		rc_feel_rp = RC_FEEL_RP_MEDIUM;
		//定高的参数控制------------------
		poshold_brake_rate = POSHOLD_BRAKE_RATE_DEFAULT;
		poshold_brake_angle_max = POSHOLD_BRAKE_ANGLE_DEFAULT;
		//---------------------------------
		land_repositioning = LAND_REPOSITION_DEFAULT;
		fs_ekf_action = FS_EKF_ACTION_DEFAULT;
		fs_ekf_thresh = FS_EKF_THRESHOLD_DEFAULT;
		fs_crash_check = 1;
		//rc_1 = RC_Channel;
		//rc_2 = RC_Channel;
		//rc_3 = RC_Channel;
		...
		//rc_8 = RC_Channel_aux;
		//rc_9 = RC_Channel_aux;
		//rc_10 = RC_Channel_aux;
		//rc_11 = RC_Channel_aux;
		//rc_12 = RC_Channel_aux;
		//rc_13 = RC_Channel_aux;
		//rc_14 = RC_Channel_aux;
		rc_speed = RC_FAST_SPEED;
		acro_rp_p = ACRO_RP_P;
		acro_yaw_p = ACRO_YAW_P;
		acro_balance_roll = ACRO_BALANCE_ROLL;
		acro_balance_pitch = ACRO_BALANCE_PITCH;
		acro_trainer = ACRO_TRAINER_LIMITED;
		acro_expo = ACRO_EXPO_DEFAULT;
		/*pid_rate_roll = AC_PID;
		pid_rate_pitch = AC_PID;
		pid_rate_yaw = AC_PID;
		pi_vel_xy = AC_PI_2D;
		p_vel_z = AC_P;
		pid_accel_z = AC_PID;
		p_stabilize_roll = AC_P;
		p_stabilize_pitch = AC_P;
		p_stabilize_yaw = AC_P;
		p_alt_hold = AC_P;
		p_pos_xy = AC_P;
		relay = AP_Relay;
		landinggear = AP_LandingGear;
		compass = Compass;
		ins = AP_InertialSensor;
		wp_nav = AC_WPNav;
		circle_nav = AC_Circle;
		attitude_control = AC_AttitudeControl;
		pos_control = AC_PosControl;
		ahrs = AP_AHRS;
		battery = AP_BattMonitor;
		BoardConfig = AP_BoardConfig;
		barometer = AP_Baro;
		gps = AP_GPS;
		scheduler = AP_Scheduler;
		motors = AP_MotorsMulticopter;
		rcmap = RCMapper;
		EKF = NavEKF;
		EKF2 = NavEKF2;
		mission = AP_Mission;
		rssi = AP_RSSI;
		rpm_sensor = AP_RPM;*/
		autotune_axis_bitmask = 7;
		autotune_aggressiveness = 0.1f;
		autotune_min_d = 0.004f;
    }

	AP_Int8 get_failsafe_throttle_enable();
	AP_Int16 get_failsafe_throttle_value();
};

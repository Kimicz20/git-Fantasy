...
const AP_Scheduler::Task Copter::scheduler_tasks[] = {
    //SCHED_TASK(rc_loop,                4,    130),		//读取遥控信号
	{ "rc_loop",4,130 }
    //SCHED_TASK(throttle_loop,          8,     75),
    //SCHED_TASK(update_GPS,             8,    200),		//更新GPS信号
    //SCHED_TASK(update_batt_compass,   40,    120),		//更新电压值
    //SCHED_TASK(read_aux_switches,     40,     50),
    //SCHED_TASK(arm_motors_check,      40,     50),		//进行加解锁操作，并且自动加锁。
    //SCHED_TASK(auto_trim,             40,     75),
    //SCHED_TASK(update_altitude,       40,    140),		//更新高度信息
...
void Copter::setup()
{
    ...
    //init_ardupilot();
	//上面的init_ardupilot()有下面函数暂时代替，在循环调度里完成rc_loop功能。
	init_rc_in();
	//std::cout << ARRAY_SIZE(scheduler_tasks);
	//std::cout << scheduler_tasks[0].name << std::endl << scheduler_tasks[0].interval_ticks << std::endl << scheduler_tasks[0].max_time_micros << std::endl;
    // initialise the main loop scheduler
    scheduler.init(&scheduler_tasks[0], ARRAY_SIZE(scheduler_tasks));

    // setup initial performance counters
    perf_info_reset();
	fast_loopTimer = clock() * 1000; //hal.scheduler->micros();
}
...
void Copter::loop()
{
    ...
    uint32_t time_available = (timer + MAIN_LOOP_MICROS) - clock()*1000;
...
void Copter::rc_loop()
{
    ...
    // -----------------------------------------
    this->supt->setHandler();
    this->supt->setAlarm(5);
    // 测试执行前 设置当前测试用例
    this->supt->setCurrentTestCase();

    std::cout<<endl<<"测试执行完前 :"<<endl<<this->supt->getCurrentTestCase()->showTestCase()<<endl;

    /* 起始插桩路径记录 */
    long start = clock();
    this->supt->deal("read_radio",start,1);
    read_radio();
    /* 结束插桩路径记录 */
    long end = clock();
    this->supt->deal("read_radio",end,2);
    this->supt->deal("read_radio",(end-start),3);
    this->supt->setResultAfterExe("OK");
    std::cout<<endl<<"测试执行完后 :"<<endl<<this->supt->getCurrentTestCase()->showTestCase()<<endl;
    this->supt->putTestCasesInMem();

...
Compass::read(void)
{
    ...
    for (uint8_t i=0; i < COMPASS_MAX_INSTANCES; i++) {
        _state[i].healthy = (clock() - _state[i].last_update_ms < 500);
    }
    return healthy();
}
...
void Compass::setHIL(uint8_t instance, const Vector3f &mag)
{
    ...
    _hil.healthy[instance] = true;
	_state[instance].last_update_usec = clock()*1000;
}

        ...
        _hil_mode(false)
{
	...
	//初始化用户可设定的参数
	sensors[0].ground_pressure = 0;
	sensors[0].ground_temperature = 0;
	_alt_offset = 0;
	_primary_baro = 0;
}
...
void AP_Baro::set_external_temperature(float temperature)
{
    ...
    _last_external_temperature_ms = clock();
}
...
float AP_Baro::get_calibration_temperature(uint8_t instance) const
{
    ...
    if (_last_external_temperature_ms != 0 && clock() - _last_external_temperature_ms < 10000) {
...
void AP_Baro::init(void)
{
	...
	sensors[0].ground_pressure = 0;
	sensors[0].ground_temperature = 0;
	std::cout << "Baro: init Done" << std::endl;
}

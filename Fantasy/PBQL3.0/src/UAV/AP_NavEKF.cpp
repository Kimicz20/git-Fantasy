    ...
    gndEffectBaroScaler(4.0f)      // scaler applied to the barometer observation variance when operating in ground effect
{
	...
	//用户可设定参数初始化
	_gpsHorizVelNoise = VELNE_NOISE_DEFAULT;
	_gpsVertVelNoise = VELD_NOISE_DEFAULT;
	_gpsHorizPosNoise = POSNE_NOISE_DEFAULT;
	_baroAltNoise = ALT_NOISE_DEFAULT;
	_magNoise = MAG_NOISE_DEFAULT;
	_easNoise = 1.4f;
	_windVelProcessNoise = 0.1f;
	_wndVarHgtRateScale= 0.5f;
	_gyrNoise = GYRO_PNOISE_DEFAULT;
	_accNoise=ACC_PNOISE_DEFAULT;
	_gyroBiasProcessNoise = GBIAS_PNOISE_DEFAULT;
	_accelBiasProcessNoise = ABIAS_PNOISE_DEFAULT;
	_magEarthProcessNoise= MAGE_PNOISE_DEFAULT;
	_magBodyProcessNoise = MAGB_PNOISE_DEFAULT;
	_msecVelDelay = 220;
	_msecPosDelay = 220;
	_fusionModeGPS = 0;
	_gpsVelInnovGate= VEL_GATE_DEFAULT;
	_gpsPosInnovGate=POS_GATE_DEFAULT;
	_hgtInnovGate = HGT_GATE_DEFAULT;
	_magInnovGate= MAG_GATE_DEFAULT;
	_tasInnovGate = 10;
	_magCal= MAG_CAL_DEFAULT;
	_gpsGlitchAccelMax = GLITCH_ACCEL_DEFAULT;
	_gpsGlitchRadiusMax=GLITCH_RADIUS_DEFAULT;
	_gndGradientSigma = 2;
	_flowNoise = FLOW_NOISE_DEFAULT;
	_flowInnovGate = FLOW_GATE_DEFAULT;
	_msecFLowDelay = FLOW_MEAS_DELAY;
	_rngInnovGate = 5;
	_maxFlowRate= 2.5f;
	_fallback = 1;
	_altSource = 1;
...
{
    return  _ahrs->get_correct_centrifugal();
}

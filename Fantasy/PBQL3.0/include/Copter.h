...
public:
	...
	/*---------手动/自动 加解锁------------*/
	int16_t arming_counter;
	int16_t arm_check_throttle;
	int16_t arm_check_yaw;
	uint8_t arm_check_motorstate;
	bool mode_manual_throttle;
	bool init_arm_done;
	/*------------------------------*/
	...
	struct {
		...
		int8_t switch_position;			    // desired switch position
		uint32_t now_time;
	...
	void set_failsafe_radio_counter(int);
	int8_t get_desired_mode();
	void set_control_switch_last_edge_time();
	void reset_arming_counter();
	void increase_arming_counter();
	/*-------------------------------------*/
	...
	void set_throttle_zero_flag(int16_t);
	...
	void report_version();
	void read_control_switch(uint32_t t);
